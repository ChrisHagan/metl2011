...
function submitSimpleSearch(url, query)
{
    ...
    if ($("#frmsrch").attr('action').indexOf(url) > -1) {
...
function InitASrch()
{
    ...
    var qstr = (queryString['query']) ? queryString['query'] : '';
    ...
    if (document.getElementsByName)
    {
        ...
        for (j = 0; j < el.length; j++)
        {
            if (el[j].disabled)
                continue;
            el[j]._prevvalue = null;            
            tval = optcookie["ad" + el[j].value];
            if (tval)
            {

                if (tval == '0') el[j].checked = false;
...
function CreateSearchOptions(id, acWidth)
{
    ...
    $(optSubContainer).width($('#gsfx_bsrch_divQuery').get(0).offsetWidth+2);
...
function ResizeSiteOpts(listDiv) 
{
    ...
    {
        optContainer.style.top = $(listDiv).get(0).clientHeight;
    }
    else 
    {
...
function ChangeBackground(element, isMouseOver) 
{
    var hoverImgPath = '/library/images/support/cn/';
    if (isMouseOver) 
    {
        hoverImgPath += "ss_dd_select2.png";
    }
    else
    {
        hoverImgPath += "ss_dd_select1.png";
    }

    if (isIE6) 
    {
        $('#gsfx_cat_sel_ie6').css({ 'filter': 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'' + hoverImgPath + '\'' });
    }
    else 
    {
        $(element).css({ 'background-image': 'url(' + hoverImgPath + ')' });
    }
}


function SetCatLangPos() 
{
    if (avlCatalogs.length == 0) 
    {
        return;
    }
    $('#gsfx_cat_sel_cntnr').get(0).style.top = $('#gsfx_cat_sel').height() - 5;
    $('#gsfx_cat_sel_cntnr').width(($('#gsfx_bsrch_query').parent().width() / 2) + $('#gsfx_cat_sel').width()+15);
}

function ShowHideLangSel(event) 
{
    var targetEl = !(event.target) ? event.srcElement : event.target;
    ...
    if($('#gsfx_cat_sel_cntnr').css('display') == 'none')
    {
        $(optContainer).css({ 'display': 'none' });
        $('#gsfx_cat_sel_cntnr').css({ 'display': 'block' });
    }
    else 
    {
        HideLangSel();
...
function HideLangSel()
{
    ...
    $('#gsfx_cat_sel_cntnr').css({ 'display': 'none' });
}
