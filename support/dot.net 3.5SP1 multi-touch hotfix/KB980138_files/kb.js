...
var originalDiv = new Object();
function kb_lightBoxLoad() {
    var lb = document.getElementById("lb");
    if (!lb)
        return;   

    var c1 = $("#gsfx_brnd_CenterContent");
    //gsfx_brnd_CenterContent doesn't exit when fr=1. don't pop-up light box in this case.
    if (c1.length == 0) {
        return;
    }
    ...
    originalDiv.height = c1.css("height");
    originalDiv.overflow = c1.css("overflow");
    ...
    if ($.browser.msie) {
        ...
        //fire fox will display the scroll bar when height = 100%
        c1.css("height", cHeight);
    }
...
function SetLightBoxPosition() {
    ...
    lb.style.left = lbLeftPosition;
    lb.style.top = lbTopPosition;
}
...
function CloseLightBox(obj) {
    ...
    var c1 = $("#gsfx_brnd_CenterContent");
    c1.css("height", '');
    c1.css("overflow", originalDiv.overflow);
...
function kb_object()
{
    ...
    this.makewhitespace = function() {
        ...
        var oWid, nWid = 0;
        for (var i = 0; i < kbdivs.length; i++) {
            nWid = kbdivs[i].getAttribute('nWidth');
            oWid = kbdivs[i].getAttribute('oWidth');
